...
export class Choice {
	...
	private async init(): Promise<void> {
		...
		if (userInfo) {
			try {
				const result: TestResultType[] | DefaultResponseType = await CustomHttp.request(
	...
	private processQuizzes(): void {
		...
		if (this.quizzes && this.quizzes.length > 0 && choiceOptionsElement) {
			this.quizzes.forEach((quiz: QuizListType) => {
				...
				choiceOptionElement.className = 'choice__option option'
				...
				choiceOptionTextElement.className = 'option__text'
				choiceOptionTextElement.innerText = quiz.name

				const choiceOptionArrowElement: HTMLElement | null = document.createElement('div')
				choiceOptionArrowElement.className = 'option__arrow'
				...
				if (this.testResult) {
					...
					if (result) {
						...
						choiceOptionResultElement.className = 'option__result'
						choiceOptionResultElement.innerHTML =
							'<div>Результат</div><div>' +
							result.score +
							'/' +
							result.total +
							'</div>'
